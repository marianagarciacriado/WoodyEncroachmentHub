...
#### Script 2. Woody cover change map + density plots
#### Mariana Garcia 
#### November 2017
...
## Loading data ----
cover.msc <- read.csv("scripts/users/mgarciacriado/encroachment_paper/final_scripts/mastersheets/cover_ms_clean.csv")
...
## Create map of woody cover change ---- 
...
#### Density plots of woody cover change ----
...
## Figure 1. Map and density plots ----

# Vertical alignment with cowplot
...
ggplot2::ggsave(map_dens_panel, filename = "scripts/users/mgarciacriado/encroachment_paper/final_scripts/figures/Figure_1.png", 
                width = 80, height = 35, units = "cm")


## Rates models and means ----
...
## Calculate percentages per trend and biome ----

# Percentage of increase records in T&S
pc_inc_tt <- (length(tundra_increase$Plot.ID)/length(cover.msc$Plot.ID))*100
pc_inc_tt #19.56% records of the total records (savannah+tundra) are increasing tundra records

pc_inc_t <- (length(tundra_increase$Plot.ID)/length(dp_tun$Plot.ID))*100
pc_inc_t #68.05% records of tundra records are increasing tundra records (TEXT)

pc_inc_st <- (length(savanna_increase$Plot.ID)/length(cover.msc$Plot.ID))*100
pc_inc_st #48.3% records of the total records (savannah+tundra) are increasing savannah records

pc_inc_s <- (length(savanna_increase$Plot.ID)/length(dp_sav$Plot.ID))*100
pc_inc_s #67.78% records of savanna records are increasing savanna records (TEXT)


# Percentage of decrease records in T&S
pc_dec_tt <- (length(tundra_decrease$Plot.ID)/length(cover.msc$Plot.ID))*100
pc_dec_tt #5.96% records of the total records (savannah+tundra) are decreasing tundra records
...
pc_dec_st #18.82% records of the total records (savannah+tundra) are decreasing savannah records
...
# Percentage of stable records in T&S
pc_sta_tt <- (length(tundra_stable$Plot.ID)/length(cover.msc$Plot.ID))*100
pc_sta_tt #3.21% records of the total records (savannah+tundra) are stable tundra records
...
pc_sta_st #4.13% records of the total records (savannah+tundra) are stable savannah records
...
## Calculate mean value per trend and biome ----

# Mean increase value in T&S
mean_inc_t <- tundra_increase %>% summarise(mean = mean(Annual.rate), sd = sd(Annual.rate))
mean_inc_t # 0.37% ± 0.5 is the mean increase value in the tundra (TEXT)

mean_inc_s <- savanna_increase %>% summarise(mean = mean(Annual.rate), sd = sd(Annual.rate))
mean_inc_s # 0.63 ± 0.82 is the mean increase value in the savanna (TEXT)


# Mean stable value in T&S
mean_stab_t <- tundra_stable %>% summarise(mean = mean(Annual.rate), sd = sd(Annual.rate))
mean_stab_t # 0.001% ± 0.003 is the mean stable value in the tundra (TEXT)

mean_stab_s <- savanna_stable %>% summarise(mean = mean(Annual.rate), sd = sd(Annual.rate))
mean_stab_s # 0.0003 ± 0.005% is the mean stable value in the savanna (TEXT)


# Mean decrease value in T&S
mean_dec_t <- tundra_decrease %>% summarise(mean = mean(Annual.rate), sd = sd(Annual.rate))
mean_dec_t # -0.36% ± 0.75 is the mean decrease value in the tundra (TEXT)

mean_dec_s <- savanna_decrease %>% summarise(mean = mean(Annual.rate), sd = sd(Annual.rate))
mean_dec_s # -0.4 ± 0.45 is the mean decrease value in the savanna (TEXT)


## Calculate overall absolute cover value per biome ----

# Mean value in the tundra (overall)
mean_tun <- dp_tun %>% summarise(mean = mean(Annual.rate), sd = sd(Annual.rate))
mean_tun # 0.18% ± 0.61 is the mean of cover values in the tundra (TEXT)

# Mean value in the savanna (overall)
mean_sav <- dp_sav %>% summarise(mean = mean(Annual.rate), sd = sd(Annual.rate))
mean_sav # 0.32% ± 0.85 is the mean of cover values in the savannah (TEXT)


## Models to test differences in means
inc.means.mod <- MCMCglmm(Annual.rate ~ Biome_type, data = total_inc, 
                             nitt = 100000, burnin = 5000, thin = 30)
summary(inc.means.mod)
# significantly different increase values, CIs don't overlap
save(inc.means.mod, file = "scripts/users/mgarciacriado/encroachment_paper/final_scripts/models/fixed/inc.means.mod.RData")

stab.means.mod <- MCMCglmm(Annual.rate ~ Biome_type, data = total_stab, 
                             nitt = 100000, burnin = 5000, thin = 30)
summary(stab.means.mod)
# stables are not significantly different, CIs overlap
save(stab.means.mod, file = "scripts/users/mgarciacriado/encroachment_paper/final_scripts/models/fixed/stab.means.mod.RData")

dec.means.mod <- MCMCglmm(Annual.rate ~ Biome_type, data = total_dec, 
                             nitt = 100000, burnin = 5000, thin = 30)
summary(dec.means.mod)
# decreases are not significantly different, CIs overlap
save(dec.means.mod, file = "scripts/users/mgarciacriado/encroachment_paper/final_scripts/models/fixed/dec.means.mod.RData")

overall.mod <- MCMCglmm(Annual.rate ~ Biome_type, data = cover.msc, 
                    nitt = 100000, burnin = 5000, thin = 30)
summary(overall.mod)
save(overall.mod, file = "scripts/users/mgarciacriado/encroachment_paper/final_scripts/models/fixed/overall.mod.RData")
# overall values are significantly different between biomes
...
## Largest number of increasing records relative to each continent ----
...
## Number of records captured by different methods ----
method.sum <- cover.msc %>% group_by(Method) %>% count(Method)

## Number of records before 1979 and after 2013 (available climatic timeframe) ----
start.1919 <- filter(cover.msc, Start_year < 1979) #230 records
end.2013 <- filter(cover.msc, End_year > 2013) #344 records
